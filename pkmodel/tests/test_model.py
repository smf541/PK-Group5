...
class ModelTest(unittest.TestCase):
    ...
    def test_create(self):
        ...
        with self.assertRaises(TypeError):
            model = pk.Model()
        model = pk.Model('nametest')
        self.assertEqual(model.name, 'nametest')
    ...
    def test_create_initvalues(self):
        ...
        """
        model = pk.Model( 'nametest', V_c=1.1, CL=1.2, Ka=1.4 )
        self.assertEqual(model.name, 'nametest')
    ...
    def test_compartments(self):
        ...
        added = model.add_compartment(1.0,1.1)
        self.assertEqual(True, added)
        with self.assertRaises(TypeError):
            model.add_compartment('a','b')
        self.assertEqual(model.list, [[1.0,1.1]])
        model.add_compartment(1.2,1.3)
        self.assertEqual(model.list, [[1.0,1.1],[1.2,1.3]])
